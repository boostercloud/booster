...
function requireRocket(rocketPackageName: string): (params: unknown) => unknown {
  return require(rocketPackageName).default
}

export function loadRocket<TProviderContext>(
  rocketDescriptor: RocketDescriptor<TProviderContext>
): Rocket<TProviderContext> {
  const rocketBuilder = requireRocket(rocketDescriptor.packageName) as (params: unknown) => Rocket<TProviderContext>
  if (!rocketBuilder)
    throw new Error(
  ...
  if (!rocket?.mount || !rocket?.unmount)
    throw new Error(
