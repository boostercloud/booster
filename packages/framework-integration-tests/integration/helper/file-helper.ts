...
export const loadFixture = (fixturePath: string, replacements?: Array<Array<string>>): string => {
  ...
  return (
    replacements?.reduce(
      (previousContents: string, replacement: string[]): string =>
        previousContents.split(replacement[0]).join(replacement[1]),
      template
    ) ?? template
  )
}

export const readFileContent = (filePath: string): string => readFileSync(filePath, 'utf8')
...
export const readPIDFor = (sandboxPath: string): number => {
  ...
  return Number.parseInt(readFileSync(pidFile).toString(), 10)
}

export { readdirSync as dirContents, existsSync as fileExists } from 'fs'
