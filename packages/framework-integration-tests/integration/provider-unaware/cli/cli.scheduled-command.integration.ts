...
describe('Scheduled Command', () => {
  ...
  context('Valid scheduled command', () => {
    it('should create a new scheduled command', async () => {
      ...
      const { stdout } = await command(`${cliPath} new:scheduled-command CheckCart`, {
        cwd: scheduledCommandSandboxDir,
      })
  ...
  context('Invalid scheduled command', () => {
    describe('missing scheduled command name', () => {
      it('should fail', async () => {
        const { stderr } = await command(`${cliPath} new:scheduled-command`, { cwd: scheduledCommandSandboxDir })
