  ...
  describe('configureScheduler', () => {
    ...
    it('should accept all kinds of cron expressions', () => {
      ...
      const scheduledOnArb = fc
        ...
        .filter((t) =>
          isValidCron(`${t.minute} ${t.hour} ${t.day} ${t.month} ${t.weekDay}`, { alias: true, allowBlankDay: true })
        )

      fc.assert(
        fc.property(scheduledOnArb, (scheduledOn) => {
          mockConfig = {
            scheduledCommandHandlers: {
              cmd: {
                class: fake(),
          ...
          expect(scheduler.scheduleJob).to.have.been.calledWithMatch('cmd', expectedCron)
        })
      )
    })
  })
