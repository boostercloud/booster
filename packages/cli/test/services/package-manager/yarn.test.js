...
const yarn_impl_1 = require("../../../src/services/package-manager/yarn.impl");
...
describe('PackageManager - Yarn Implementation', () => {
    ...
    it('run arbitrary scripts from package.json', async () => {
        ...
        await (0, effect_1.unsafeRunEffect)(mapEffError(effect), {
            layer: effect_1.Layer.using(testLayer)(yarn_impl_1.YarnPackageManager),
            onError: (0, errors_1.guardError)('An error ocurred'),
        });
        (0, expect_1.expect)(TestProcess.fakes.exec).to.have.been.calledWith(`yarn run ${script} ${args.join(' ')}`);
    });
    describe('when the `compile` script exists', () => {
        ...
        it('run the `compile` script', async () => {
            const testLayer = effect_1.Layer.all(TestFileSystemWithCompileScript.layer, TestProcess.layer);
            const effect = (0, effect_1.gen)(function* ($) {
            ...
            (0, expect_1.expect)(TestProcess.fakes.exec).to.have.been.calledWith('yarn run compile');
        });
    });
    describe('when the `compile` script does not exist', () => {
        it('run the `build` script', async () => {
            ...
            (0, expect_1.expect)(TestProcess.fakes.exec).to.have.been.calledWith('yarn run build');
        });
    });
    it('can set the project root properly', async () => {
        ...
        (0, expect_1.expect)(CwdTestProcess.fakes.exec).to.have.been.calledWith('yarn run script', projectRoot);
    });
    it('can install production dependencies', async () => {
        ...
        (0, expect_1.expect)(TestProcess.fakes.exec).to.have.been.calledWith('yarn install --omit=dev --omit=optional --no-bin-links');
    });
    it('can install all dependencies', async () => {
        ...
        (0, expect_1.expect)(TestProcess.fakes.exec).to.have.been.calledWith('yarn install');
    });
});
