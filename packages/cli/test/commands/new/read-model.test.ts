    ...
    describe('ReadModel', () => {
        ...
        describe('Created correctly', () => { 
            ...
            it('creates ReadModel with a number field projecting Post:id', async () => {
                await new ReadModel([readModelName, '--fields', 'quantity:number','--projects','Post:id'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: projectingReadModelImports,
                    name: readModelName,
                    fields: [{ name: 'quantity', type: 'number' }],
            ...
            it('creates ReadModel with UUID field projecting Post:id', async () => {
                await new ReadModel([readModelName, '--fields', 'identifier:UUID','--projects','Post:id'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: projectingReadModelImports,
                    name: readModelName,
                    fields: [{ name: 'identifier', type: 'UUID' }],
            ...
            it('creates ReadModel with multiple fields', async () => {
                await new ReadModel([readModelName, '--fields', 'title:string','quantity:number','identifier:UUID'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: defaultReadModelImports,
                    name: readModelName,
            ...
            it('creates ReadModel with multiple fields projecting Post:id', async () => {
                await new ReadModel([readModelName, '--fields', 'title:string','quantity:number','identifier:UUID','--projects','Post:id'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: projectingReadModelImports,
                    name: readModelName,
                    fields: [{ name: 'title', type: 'string' },{ name: 'quantity', type: 'number' },{ name: 'identifier', type: 'UUID' }],
            ...
            it('creates ReadModel with multiple fields projecting Post:id and Comment:id', async () => {
                await new ReadModel([readModelName, '--fields', 'title:string','quantity:number','identifier:UUID','--projects','Post:id','Comment:id'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: projectingTwoReadModelImports,
                    name: readModelName,
                    fields: [{ name: 'title', type: 'string' },{ name: 'quantity', type: 'number' },{ name: 'identifier', type: 'UUID' }],
                    projections: [{entityName: 'Post', entityId: 'id'}, {entityName: 'Comment', entityId: 'id'}]
                })
                expect(fs.outputFile).to.have.been.calledWithMatch(readModelPath,renderedReadModel)
            })
        })
        ...
        describe('displays an error', () => { 
            ...
            it('with empty ReadModel name', async () => {
                ...
                expect(console.error).to.have.been.calledWith("You haven't provided a read model name, but it is required, run with --help for usage")
            })
            ...
            it('with empty fields and projection', async () => {
                ...
                expect(exceptionThrown).to.be.equal(true)
                expect(exceptionMessage).to.contain('Error: Error parsing field --projects. Fields must be in the form of <field name>:<field type>')
            })
            ...
            it('with projection with no entity id', async () => {
                ...
                expect(exceptionThrown).to.be.equal(true)
                expect(exceptionMessage).to.contain('Error parsing projection Post. Projections must be in the form of <entity name>:<entity id>')
            })
        ...
        describe('should display an error but is not currently being validated', () => { 
            ...
            it('with repeated fields', async () => {
                await new ReadModel([readModelName, '--fields', 'title:string','title:string','quantity:number'], {} as IConfig).run()
                const renderedReadModel = Mustache.render(templates.readModel, {
                    imports: defaultReadModelImports,
                    name: readModelName,
