    ...
    describe('Entity', () => {
        ...
        describe('Created correctly', () => { 
            ...
            it('creates Entity with multiple fields', async () => {
                await new Entity([entityName, '--fields', 'title:string','quantity:number','identifier:UUID'], {} as IConfig).run()
                const renderedEntity = Mustache.render(templates.entity, {
                    imports: defaultEntityImports,
                    name: entityName,
            ...
            it('creates Entity with multiple fields reducing PostCreated', async () => {
                await new Entity([entityName, '--fields', 'title:string','quantity:number','identifier:UUID','--reduces','PostCreated'], {} as IConfig).run()
                const renderedEntity = Mustache.render(templates.entity, {
                    imports: reducingEntityImports,
                    name: entityName,
                    fields: [{ name: 'title', type: 'string' },{ name: 'quantity', type: 'number' },{ name: 'identifier', type: 'UUID' }],
            ...
            it('creates Entity with multiple fields reducing PostCreated and CommentCreated', async () => {
                await new Entity([entityName, '--fields', 'title:string','quantity:number','identifier:UUID','--reduces','PostCreated','CommentCreated'], {} as IConfig).run()
                const renderedEntity = Mustache.render(templates.entity, {
        ...
        describe('displays an error', () => { 
            ...
            it('with empty Entity name', async () => {
                ...
                expect(console.error).to.have.been.calledWith("You haven't provided an entity name, but it is required, run with --help for usage")
            })
            ...
            it('with empty fields and reduces', async () => {
                ...
                expect(exceptionThrown).to.be.equal(true)
                expect(exceptionMessage).to.contain('Error: Error parsing field --reduces. Fields must be in the form of <field name>:<field type>')
            })
        ...
        describe('should display an error but is not currently being validated', () => { 
            ...
            it('with repeated fields', async () => {
                await new Entity([entityName, '--fields', 'title:string','title:string','quantity:number'], {} as IConfig).run()
                const renderedEntity = Mustache.render(templates.entity, {
                    imports: defaultEntityImports,
                    name: entityName,
