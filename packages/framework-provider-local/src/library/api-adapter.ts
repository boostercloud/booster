...
export type APIResult =
  | { status: 'success'; result: unknown; headers?: Record<string, string> }
  | { status: 'failure'; code: number; title: string; reason: string }

export async function requestSucceeded(body?: any, headers?: Record<string, string>): Promise<APIResult> {
  return {
    status: 'success',
    result: body,
    headers: {
