...
Booster provides an infrastructure layer out of the box with sensible defaults that you can use for rapid development, but f you want to have control over the infrastructure deployed to your cloud provider or start from scratch with a fully customized infrastructure implementation. Booster layered architecture allows you to do so by implementing a provider package. 

## How do Booster cloud providers work?

Clouds providers have to implement two specific interfaces: 
-  **Infrastructure:** This interface is used during deployment to create, destroy and configurate all the infrastructure necessary to run the application. 
...
The provider infrastructure interface by convention is implemented in a package ending with `-infrastructure` name like the `framework-provider-aws-infrastructure`.  As it has been commented, this package 
includes all the necessary to deploy and configure cloud elements for running your application. For instance in the case of AWS, this package is in charge of deploy the DynamoDB for your event store, 
create all the lambdas, and configure all the API gateway configuration for your application.

Internally the infrastructure package interface is composed by two methods:
...
For the implementation of the infrastructure interface you can use any Infrastructure as Code tool provided by cloud providers or third parties.
...
The runtime interface is divided in different sections:
