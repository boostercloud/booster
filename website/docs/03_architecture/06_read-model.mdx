...
You can use a read model query as a join key to get all the read models that match the query. Example:
...
In this read model, since the `CarModel` entity doesn't have a field that matches the `id` field of `CarPurchasesReadModel`, this projection can use a read model query join key to get all the `CarPurchasesReadModel` instances for a given `CarModel`.
