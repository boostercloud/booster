...
This projection uses a ReadModel query JoinKey to get all the CarOwnerModelReadModel instances for a given CarModel. In this case, the `updateWithModel` function
...
> [!NOTE] If no read model matches the query, the `updateWithModel` function will be called with `readModelId` set to `undefined`.
